...
function delta (plane, path) {
    ...
    return function (i) {
      return function (t) {
        ...
        return 'translate(' + p.x + ',' + p.y + ') scale(' + s + ') rotate(' + r + ", " + (centre.x + centre.width * s * 0.75) + ", " + (centre.y + centre.height * s * 0.6) + ")";

      }
    }
}

function display_flight (origin, destination, path3) {
...
function show_flights (path3, point_pairs) {
    //routes = [['AA','BB'],['BB','CC'],['CC','DD'],['DD','EE']]
    function flyBatch () {
      let end = start + point_pairs.length
      const pairs = point_pairs.slice(start, end)
      pairs.forEach((pair, idx) => {
        display_flight(pair[0], pair[1], path3)
      })
      start = end
